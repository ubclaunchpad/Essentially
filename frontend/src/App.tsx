...
import { ThemeContent, lightDarkOnClick } from "./context/ThemeContent"
...
function App() {
      ...
      return (
        <div className="settings">
          <div className="dropdown">
            <button className={theme.theme.toLowerCase()}>Settings</button>
            <div>
              <button className={theme.theme.toLowerCase()}>Summary Length</button>
              <button className={theme.theme.toLowerCase()}>Appearance</button>
              <button
  ...
  return (
    <div className="popup">
      <div className={"main" + " " + theme.theme.toLowerCase()}>
        <div className="start">
          <nav>
            <h1>{theme.theme}</h1>
            <button onClick={() => setShowSettings((prev) => !prev)}>
              ...
              >
                <path
                  ...
                  fill={theme.theme == 'Dark' ? "#D9D9D9" : 'black'}
                />
              </svg>
            </button>
          </nav>
          ...
          <section className="actions">
            {!text.body.length && (
              <button className={theme.theme.toLowerCase()} onClick={() => callExtension()}>SUMMARIZE</button>
            )}
          </section>
        </div>
      </div>
